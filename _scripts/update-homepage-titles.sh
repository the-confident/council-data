...
while IFS=$'\t' read -r name url; do
	...
	file="$homepages_dir/$slug.html"

	if [[ ! -f "$file" ]]; then
		echo "Warning: no homepage file found for $name at $file" >&2
		((missing += 1))
		continue
	fi
...
if [[ "$processed" -eq 0 ]]; then
	echo "Error: no valid title updates found in $homepages_dir" >&2
	exit 1
fi
...
if [[ "$missing" -gt 0 ]]; then
	echo "Skipped $missing councils due to missing homepage files or <title> values" >&2
